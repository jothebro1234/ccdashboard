...
/* Sort curriculum/event rows newest-first. Primary key is the row's own DueDate/EventDate
   (r[1]) since that's present on virtually every row and is what "recency" visibly means
   in a dated list — raw sheet row order is NOT a reliable proxy for it (sheets get
   manually re-sorted/edited outside the app). Rows with no date yet (a duration-mode
   curriculum assignment that hasn't started) fall back to their PostedAt timestamp
   (col P=r[15], stamped at creation), then finally to relative row order. */
function sortByRecency(rows) {
    return rows
        .map((r,i)=>({r,i,t:Date.parse(r[1]||'')||Date.parse(r[15]||'')||0}))
...
async function loadVolunteerData(name) {
    ...
    S.data.myEventRegistrations=sortByRecency(S.data.upcomingEvents.filter(r=>{
        const reg=(r[7]||'').split(',').map(n=>n.trim().toLowerCase());
        const cred=(r[3]||'').split(',').map(n=>n.trim().toLowerCase());
        return reg.includes(lower)&&!cred.includes(lower);
    })); // newest posted first
...
function renderCurrList(filter) {
    ...
    if(!listEl)return;
    const assignments=sortByRecency(S.data.curriculum||[]); // newest posted first
...
function dirPostAssignHTML() {
    ...
    const existingCards=sortByRecency(existing).map(r=>{
        const filled=(r[7]||'').split(',').map(n=>n.trim()).filter(Boolean).length;
        const maxVols=parseInt(r[6])||0;
        const done=isCompleted(r[1]);
        const locked=isClosed(r[5],r[1],r[14]);
...
function dirGiveHoursHTML() {
    ...
    const assignments=sortByRecency(rawAssignments); // newest posted first
...
function dirRecordEventHTML() {
    const events=sortByRecency(S.data.events||[]).slice(0,10);
    return `
        <div style="display:grid;grid-template-columns:1fr 1.1fr;gap:20px;align-items:start">
            <div class="card">
...
function renderActivitiesList(filter) {
    ...
    const assignments=sortByRecency(S.data.curriculum||[]);
    const upcomingEvs=sortByRecency(S.data.upcomingEvents||[]);
    const lower=(S.user?.name||'').toLowerCase();
...
function dirGiveEventHoursHTML() {
    ...
    const events=sortByRecency(rawEvents);
...
function viewMyChapter() {
    ...
    const chapEvents=sortByRecency(S.data.upcomingEvents||[]).filter(r=>(r[10]||'').trim().toLowerCase()===userSchool); // newest posted first
